...
var
    on_screen_x, on_screen_y: boolean;
begin
    on_screen_x := (x + dx > 0) and (x + dx < ScreenWidth - length(msg));
    on_screen_y := (y + dy > 0) and (y + dy < ScreenHeight);
    if on_screen_x and on_screen_y then
    begin
        HideMessage(x, y, msg);
        x := x + dx;
        y := y + dy;
